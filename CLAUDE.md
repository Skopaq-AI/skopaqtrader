...
**Jev decisions** (`skopaq/llm/jev.py`, off unless `SKOPAQ_JEV_ENABLED=true`): Jev reads the Portfolio Manager's decision and sets the signal's confidence to its probability for that action (a confident contradiction → HOLD), and makes the sell analyst's SELL/HOLD call (SELL only at ≥ `SKOPAQ_JEV_MIN_CONFIDENCE`). Model pinned to `jev-1.13.0`. Jev is weak at numbers and dates — ask it semantic questions only; keep arithmetic in code. Any Jev failure falls back to the LLM's own answer.
